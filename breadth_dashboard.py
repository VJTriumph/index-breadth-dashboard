...
def fetch_one(symbol):
        ...
        # Fetch extra day so today's bar is included if market closed
        ...
        try:
                    ...
                    # Today's close (last bar) — this IS today's session close or live price
                    today_close = float(closes.iloc[-1])
                    # Previous trading day's close — second-to-last bar
                    prev_close  = float(closes.iloc[-2]) if len(closes) >= 2 else today_close

            # Try fast_info for a real-time last traded price first
                    try:
                                    fi    = tk.fast_info
            ...
            # changePct vs previous trading day close
