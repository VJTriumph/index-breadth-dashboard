...
def fetch_one(symbol):
    ...
    # Fetch 400 days to ensure 200-SMA + 52W history
    ...
    try:
        ...
        try:
            ...
            lp = fi.get("last_price") or fi.get("regularMarketPrice")
        ...
        # Historical breadth: compare close price at N days ago vs SMA at that point
        hist_above = {}
        ...
        # 7% proximity thresholds — use live price for today's near high/low
        ...
        return {
            "symbol":     symbol,
            "price":      price,
            "prevClose":  round(prev_eod_close, 2),
            "changePct":  chgPct,
            "sma20":      s20,
            "sma50":      s50,
            "sma200":     s200,
...
def write_crosscheck_csv(all_data, all_breadth):
    ...
    header = [
        "FetchedAt", "Index", "Symbol",
        "LivePrice", "PrevEODClose", "ChangePct",
        "SMA20", "SMA50", "SMA200",
        "AboveSMA20", "AboveSMA50", "AboveSMA200",
        "High52W", "Low52W", "NearHigh_7pct", "NearLow_7pct",
        "PriceToHigh52_pct", "PriceToLow52_pct",
        "Hist_1D_Price", "Hist_1D_SMA20", "Hist_1D_SMA50", "Hist_1D_SMA200",
        "Hist_1W_Price", "Hist_1W_SMA20", "Hist_1W_SMA50", "Hist_1W_SMA200",
        "Hist_1M_Price", "Hist_1M_SMA20", "Hist_1M_SMA50", "Hist_1M_SMA200",
        "Hist_3M_Price", "Hist_3M_SMA20", "Hist_3M_SMA50", "Hist_3M_SMA200",
        "Error"
    ]
    rows = []
    for name, stocks in all_data.items():
        for sym, s in stocks.items():
            ...
            def pct_to_low(s):
                ...
                return ""
            def hp(period, key):
                return s.get("hist", {}).get(period, {}).get(key, "")
            def h(period, key):
                return yn(s.get("hist", {}).get(period, {}).get(key))
            rows.append([
                now_ist, name, s.get("symbol", ""),
                s.get("price", ""), s.get("prevClose", ""), s.get("changePct", ""),
                ...
                hp("1D","price"), h("1D","aboveSma20"), h("1D","aboveSma50"), h("1D","aboveSma200"),
                hp("1W","price"), h("1W","aboveSma20"), h("1W","aboveSma50"), h("1W","aboveSma200"),
                hp("1M","price"), h("1M","aboveSma20"), h("1M","aboveSma50"), h("1M","aboveSma200"),
                hp("3M","price"), h("3M","aboveSma20"), h("3M","aboveSma50"), h("3M","aboveSma200"),
                s.get("error", "")
            ])
